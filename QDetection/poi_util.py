...
class flipping_label(Dataset):
    def __init__(self, dataset, indices, tar_lab, transform, seed, num_classes):
        set_seed(seed)
        self.dataset = dataset
        self.indices = indices
        self.data = copy.deepcopy(self.dataset.data)
...
def poi_dataset(Dataset, poi_methond='badnets', transform=None, tar_lab=0, poi_rates=0.2, random_seed=0, noisy=None):
    ...
    if poi_methond == 'targeted_label_filpping':
        ...
        poi_rates = (poi_rates * np.where(np.array(label) == tar_lab)[0].shape[0]) / (1 - poi_rates) / \
                    current_label.shape[0]
        ...
        posion_dataset = flipping_label(Dataset, poi_idx, tar_lab, transform, random_seed, num_classes=num_classes)
